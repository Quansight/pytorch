...
static void col2im_out_cpu_template(
  ...
  if (input.dim() == 2) {
    // Force batch
    batched_input = false;
    input.resize_({1, input_.size(0), input_.size(1)});
  }

  long batch_size = input_.size(0);
  long n_input_plane = input_.size(1);
  long n_output_plane = n_input_plane / (kernel_width * kernel_height);

  output.resize_({batch_size, n_output_plane, output_height, output_width});
  output.zero_();

  // TODO: migrate Spatial Convolution and change dispatch to
  // AT_DISPATCH_FLOATING_TYPES_AND_HALF
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "col2im_out_cpu", [&] {
...
void col2im_backward_out_cpu_template(
    ...
    IntArrayRef padding,
    IntArrayRef stride) {
  // const Tensor& grad_input = &grad_input_;
