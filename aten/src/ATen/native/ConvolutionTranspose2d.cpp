...
static void conv_transpose2d_backward_out_cpu_template(
  ...
  AT_DISPATCH_FLOATING_TYPES(
      grad_output.scalar_type(), "conv_transpose2d_out_cpu", [&] {
        // Helpers
...
std::tuple<Tensor&, Tensor&, Tensor&> conv_transpose2d_forward_out_cpu(
    Tensor& output,
    Tensor& columns,
    Tensor& ones,
    const Tensor& input,
    ...
    IntArrayRef output_padding,
    IntArrayRef dilation) {
  std::cout << "NOT IMPLEMENTED: conv_transpose2d_forward_out_cpu3"
            << std::endl;
  return std::tie(output, columns, ones);
}

Tensor& conv_transpose2d_forward_out_cpu(
    Tensor& output,
    const Tensor& input,
    const Tensor& weight,
    IntArrayRef kernel_size,
  ...
  std::cout << "NOT IMPLEMENTED: conv_transpose2d_forward_out_cpu1"
            << std::endl;
  return output;
}

std::tuple<Tensor, Tensor, Tensor> conv_transpose2d_forward_cpu(
    const Tensor& input,
    const Tensor& weight,
    IntArrayRef kernel_size,
    const Tensor& bias,
  ...
  std::cout << "NOT IMPLEMENTED: conv_transpose2d_forward_cpu" << std::endl;
  return std::tie(output, columns, ones);
}
