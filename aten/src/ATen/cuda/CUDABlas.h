...
/*
  Provides a subset of CUDA BLAS functions as templates:
    ...
    gemm<Dtype>(stream, transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
...
#define GEMM_ARGTYPES(Dtype)                                               \
  cudaStream_t stream, cublasOperation_t transa, cublasOperation_t transb, \
      int64_t m, int64_t n, int64_t k, Dtype alpha, const Dtype *a,        \
      int64_t lda, const Dtype *b, int64_t ldb, Dtype beta, Dtype *c,      \
      int64_t ldc
...
void gemm<double>(GEMM_ARGTYPES(double)) {
  ...
  AT_CUDABLAS_CHECK(cublasSetStream(handle, stream));
  AT_CUDABLAS_CHECK(cublasDgemm(
...
void gemm<at::Half>(GEMM_ARGTYPES(at::Half)) {
  ...
  float fbeta = beta;
  AT_CUDABLAS_CHECK(cublasSetStream(handle, stream));
  AT_CUDABLAS_CHECK(cublasSgemmEx(
      handle,
      transa,
      transb,
      m,
...
#define GEMV_ARGTYPES(Dtype)                                                  \
  cudaStream_t stream, cublasOperation_t trans, int64_t m, int64_t n,         \
      Dtype alpha, const Dtype *a, int64_t lda, const Dtype *x, int64_t incx, \
